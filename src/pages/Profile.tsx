import { ChevronLeft, ChevronRight, Settings, Palette, Moon, Clock, Gauge, Shield, HelpCircle, FileText, Info } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-sp-divider safe-top">
        <div className="flex items-center justify-between h-14 px-4">
          ...
          <Link to="/settings" className="p-2 rounded-full hover:bg-sp-hover transition-colors">
            <Settings className="w-5 h-5 text-sp-icon" />
          </Link>
        </div>
      </header>
      ...
      {/* Profile Card */}
      <div className="p-4">
        <div className="flex items-center gap-4 p-4 bg-card rounded-2xl border border-primary/20">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center">
            <span className="text-2xl font-bold text-primary-foreground">S</span>
          </div>
          <div>
            <h2 className="text-lg font-semibold text-foreground">ShadowPlay User</h2>
            <p className="text-sm text-muted-foreground">{savedVideos.length} saved videos</p>
          </div>
        </div>
      </div>
      ...
      {/* Menu Items */}
      <div className="mt-4">
        <h3 className="text-sm font-medium text-muted-foreground px-5 mb-2">Settings</h3>
        <div className="bg-card border-y border-sp-divider">
          {[
            ...
            { icon: Info, label: 'About ShadowPlay', path: '/about' },
          ].map((item, index, arr) => (
            <Link
              key={item.path}
              to={item.path}
              className={cn(
                ...
                index !== arr.length - 1 && 'border-b border-sp-divider'
              )}
            >
              <div className="flex items-center gap-4">
